...
var SUPABASE_URL = "https://fjsvanmveizzychvyiuv.supabase.co";
var SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZqc3Zhbm12ZWl6enljaHZ5aXV2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwODY2MDQsImV4cCI6MjA5MDY2MjYwNH0.QSePTIBlLl-HNPe9Bg8ZuZX766XowewZj2DGs3VXqak";

function dbSaveUser(user) {
  var code = "CASH-" + (user.name ? user.name.split(" ")[0].toUpperCase() : "USER") + Math.floor(Math.random()*900+100);
  fetch(SUPABASE_URL + "/rest/v1/cashdrop_users", {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY, "Prefer": "resolution=merge-duplicates" },
    body: JSON.stringify({ id: user.id, name: user.name, email: user.email, plan: user.plan || "free", balance: 0, referral_code: code })
  }).catch(function(e) { console.log("DB error", e); });
}

function dbSaveReferral(referrerId, refereeId, refereeEmail) {
  fetch(SUPABASE_URL + "/rest/v1/cashdrop_referrals", {
    method: "POST",
    headers: { "Content-Type": "application/json", "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY },
    body: JSON.stringify({ referrer_id: referrerId, referee_id: refereeId, referee_email: refereeEmail, bonus_paid: false })
  }).catch(function(e) { console.log("Ref error", e); });
}

function dbGetUserByCode(code) {
  return fetch(SUPABASE_URL + "/rest/v1/cashdrop_users?referral_code=eq." + code + "&select=*", {
    headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY }
  }).then(function(r) { return r.json(); });
}

function dbGetMyReferrals(userId) {
  return fetch(SUPABASE_URL + "/rest/v1/cashdrop_referrals?referrer_id=eq." + userId + "&select=*", {
    headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY }
  }).then(function(r) { return r.json(); });
}
...
function AuthScreen({ onAuth }) {
  ...
  function handleGoogleResponse(response) {
    ...
    try {
      ...
      saveUser(user);
      fetch("https://fjsvanmveizzychvyiuv.supabase.co/rest/v1/cashdrop_users", { method: "POST", headers: { "Content-Type": "application/json", "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZqc3Zhbm12ZWl6enljaHZ5aXV2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwODY2MDQsImV4cCI6MjA5MDY2MjYwNH0.QSePTIBlLl-HNPe9Bg8ZuZX766XowewZj2DGs3VXqak", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZqc3Zhbm12ZWl6enljaHZ5aXV2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwODY2MDQsImV4cCI6MjA5MDY2MjYwNH0.QSePTIBlLl-HNPe9Bg8ZuZX766XowewZj2DGs3VXqak", "Prefer": "resolution=merge-duplicates" }, body: JSON.stringify({ id: user.id, name: user.name, email: user.email, plan: "free", balance: 0, referral_code: "CASH-" + user.name.split(" ")[0].toUpperCase() + Math.floor(Math.random()*900+100) }) }).then(function(r) { return r.json(); }).then(function(d) { console.log("Google saved:", d); }).catch(function(e) { console.log("Error:", e); });
      setLoading(false);
      onAuth(user);
    } catch(e) {
  ...
  function handleEmailSubmit() {
    ...
    setTimeout(function() {
      ...
      if (mode === "signup") {
        ...
        saveUser(authUser);
        fetch("https://fjsvanmveizzychvyiuv.supabase.co/rest/v1/cashdrop_users", {
          method: "POST",
          headers: { "Content-Type": "application/json", "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZqc3Zhbm12ZWl6enljaHZ5aXV2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwODY2MDQsImV4cCI6MjA5MDY2MjYwNH0.QSePTIBlLl-HNPe9Bg8ZuZX766XowewZj2DGs3VXqak", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZqc3Zhbm12ZWl6enljaHZ5aXV2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwODY2MDQsImV4cCI6MjA5MDY2MjYwNH0.QSePTIBlLl-HNPe9Bg8ZuZX766XowewZj2DGs3VXqak", "Prefer": "resolution=merge-duplicates" },
          body: JSON.stringify({ id: authUser.id, name: authUser.name, email: authUser.email, plan: "free", balance: 0, referral_code: "CASH-" + authUser.name.split(" ")[0].toUpperCase() + Math.floor(Math.random()*900+100) })
        }).then(function(r) { return r.json(); }).then(function(d) { console.log("Saved:", d); }).catch(function(e) { console.log("Error:", e); });
        setLoading(false);
        onAuth(authUser);
      ...
      <div style={{ width:"100%", maxWidth:360, position:"relative", zIndex:2 }}>
        ...
        {resetSent ? (
          ...
          <>
            ...
            {mode === "signup" && (
              <input placeholder="Referral code (optional)" value={refCode}
                onChange={function(e) { setRefCode(e.target.value.toUpperCase()); }} style={INP} />
            )}
...
function AppMain({ user, onLogout }) {
  ...
  useEffect(function() {
    if (tab === "refer" && user && user.id) {
      dbGetMyReferrals(user.id).then(function(d) {
        if (d && Array.isArray(d)) { setReferrals(d); }
      }).catch(function() {});
    }
  }, [tab]);
        ...
        <div style={{ padding:"0 24px 100px" }}>
          ...
          {referrals.length === 0 ? (
            <div style={{ textAlign:"center", color:"#444", padding:"20px 0", fontSize:13 }}>
              No referrals yet. Share your link to earn $5!
            </div>
          ) : referrals.map(function(r, ridx) {
            return (
              <div key={ridx} style={{ background:"#0f0f18", border:"1px solid #1a1a1a",
                borderRadius:14, padding:"14px 16px", marginBottom:10,
                display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  <div style={{ fontWeight:700 }}>{r.referee_email}</div>
                  <div style={{ fontSize:12, color: r.bonus_paid ? "#00ffaa" : "#666", marginTop:2 }}>
                    {r.bonus_paid ? "Paid" : "Pending"}
                  </div>
                </div>
                <div style={{ color:"#00ffaa", fontWeight:900 }}>$5.00</div>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)",
          ...
          <div style={{ background:"#0e0e18", borderRadius:"24px 24px 0 0",
            ...
            <button onClick={function() {
              // 🔴 Replace with: window.location.href = "https://buy.stripe.com/YOUR_LINK";
...
function AdminDash({ onBack }) {
  var [stats, setStats] = useState([
    { label:"Total Revenue",     value:"Loading...", icon:"💰", color:"#00ffaa" },
    { label:"Total Users",       value:"...",        icon:"👥", color:"#c9a84c" },
    { label:"Pro Subscribers",   value:"...",        icon:"⭐", color:"#ff9900" },
    { label:"Free Members",      value:"...",        icon:"👤", color:"#00ffaa" },
    { label:"Referrals Made",    value:"...",        icon:"🎁", color:"#c9a84c" },
  ...
  useEffect(function() {
    ...
    fetch(SUPABASE_URL + "/rest/v1/cashdrop_users?select=*&order=created_at.desc&limit=100", {
      headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY }
    })
    .then(function(r) { return r.json(); })
    .then(function(users) {
      var total = users.length;
      var proCount = users.filter(function(u) { return u.plan === "pro"; }).length;
      var freeCount = total - proCount;
      var revenue = proCount * 4.99;

      // Fetch referrals count
      fetch(SUPABASE_URL + "/rest/v1/cashdrop_referrals?select=*", {
        headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY }
      })
      .then(function(r) { return r.json(); })
      .then(function(refs) {
        setStats([
          { label:"Est. Revenue",    value:"$" + revenue.toFixed(2), icon:"💰", color:"#00ffaa" },
          { label:"Total Users",     value:total.toString(),          icon:"👥", color:"#c9a84c" },
          { label:"Pro Subscribers", value:proCount.toString(),        icon:"⭐", color:"#ff9900" },
          { label:"Free Members",    value:freeCount.toString(),       icon:"👤", color:"#00ffaa" },
          { label:"Total Referrals", value:(refs ? refs.length : 0).toString(), icon:"🎁", color:"#c9a84c" },
          { label:"Top Deal",        value:"Uber Eats",                icon:"🔥", color:"#ff4d00" },
        ]);
